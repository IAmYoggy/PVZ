...
void zPeaShooter::act()
{
    if (this->TimerShoot <= 0)
    {
        ...
        this->TimerShoot = this->TimerShoot_max;
        zBall* pea = new zBall(scene);
        pea->setGeometry(this->x() + 55, this->y() - 60 - (qrand() % 5), 100, 210);//这里改子弹大小参数
        pea->raw = this->raw;
...
zKunShooter::zKunShooter(QWidget *parent) : zPlant(parent)
{
...
void zKunShooter::mousePressEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        qDebug() << "KunShooter clicked! Playing sound and triggering explosion...";

        // 1️⃣ 播放大招音效
        QSound::play(":/Sounds/rc/鸡你太美_2.wav");  // 确保路径正确

        // 2️⃣ 触发爆炸动画
        zAnim* boom = new zBoom(scene);
        boom->setGeometry(this->x() + 160, this->y() - 150, 400, 300);
        ...
        {
            if ((qAbs(zombie->x() - this->x() + zombie->offset + 50) < 840) && (qAbs(this->raw - zombie->raw) <= 2))
            {
                zombie->hit(1200);
            }
        }
...
void zFireTree::act()
{
    ...
    while (i < scene->FlyingObjects.count())
    {
        ...
        else if (obj->is_ball && (obj->raw == this->raw) && (qAbs(this->x() - obj->x() - 10) < 20))
        {
            zFireBall* firepea = new zFireBall(scene);
            firepea->setGeometry(obj->x(), obj->y()+60, 100, 100);
            firepea->raw = obj->raw;
            scene->FlyingObjects.append(firepea);
            delete obj;
            scene->FlyingObjects.removeAt(i);
...
void zCherryBomb::act()
{
    ...
    else
    {
        ...
        foreach (zombie, this->scene->Zombies)
        {
            if ((qAbs(zombie->x() - this->x() + zombie->offset + 50) < 340) && (qAbs(this->raw - zombie->raw) <= 2))
            {
                zombie->hit(1200);
            }
        }
